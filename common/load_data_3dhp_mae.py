...
class Fusion(data.Dataset):
    ...
    def prepare_data(self, path, train=True):
        ...
        if train == True:
            ...
            for seq in data.keys(): # ('S1 Seq1', 'S1 Seq2', ...)
                for cam in data[seq][0].keys(): # (0, 1, 2, 4, 5, 6, 7, 8)
                    anim = data[seq][0][cam] # (data_2d / data_3d)

                    subject_name, seq_name = seq.split(" ") # 'S1', 'Seq1'

                    # load 3d data 並讓每個 joint 都去減 joint 14 (root)
                    data_3d = anim['data_3d'] # (6414, 17, 3) # ((-13, -284, 3678), (-1.6, -4.5, 3.69), ...)
                    data_3d[:, :14] -= data_3d[:, 14:15]
                    data_3d[:, 15:] -= data_3d[:, 14:15]
                    out_poses_3d[(subject_name, seq_name, cam)] = data_3d
                    
                    # load 2d data
                    data_2d = anim['data_2d'] # (6414, 17, 2) # ((1018.95, 915.374), (1023.94, 1019.53), ...)
